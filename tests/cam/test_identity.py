...
from chronospear.cam import IdentityCatalog, IdentityKind, IdentityNode, NodeId


def test_identity_node_keeps_minimal_identity_shape() -> None:
    node = IdentityNode(
        node_id=NodeId("alric"),
        name=" Alric ",
        kind=IdentityKind.ENTITY,
        description=" A veteran guard. ",
    )

    assert node.node_id == NodeId("alric")
...
def test_identity_node_rejects_empty_name() -> None:
    with pytest.raises(ValueError, match="name cannot be empty"):
        IdentityNode(NodeId("x"), "  ", IdentityKind.ENTITY)
...
def test_identity_catalog_rejects_conflicting_node_id() -> None:
    ...
    catalog.add(IdentityNode(NodeId("same"), "Alric", IdentityKind.ENTITY))

    with pytest.raises(ValueError, match="already in use"):
        catalog.add(IdentityNode(NodeId("same"), "Elara", IdentityKind.ENTITY))
